...
/*   Updated: 2026/08/01 04:04:23 by alerradi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_string	valid_input(t_string str)
{
	...
	while (*digits)
	{
		if (!ft_isdigit(*digits))
			ft_error("valid_input", "argument contains a non-digit character",
				1);
		++digits;
	}
	if (ft_strlen(str) == 0)
	...
	if (ft_strlen(str) > 10)
		ft_error("valid_input", "argument exceeds maximum digit length (10)",
			1);
	return (str);
}
...
void	assign_scheduler(t_config *config, t_string arg)
{
	...
	if (strcmp(scheduler, "fifo") == 0)
		...
		ft_error("parser_input",
			"unknown scheduler type,expected *fifo* or *edf*", 1);
}
...
void	parser_input(t_config *config, int argc, char **av)
{
	...
	if (coders_count > 250)
		ft_error("parser_input",
			"number_of_coders exceeds safe thread limit (max 250)", 1);
	...
	config->num_of_req_compile = (t_uint)ft_number(av[6]);
	...
	if (config->num_of_req_compile == 0)
		exit(EXIT_SUCCESS);
}
